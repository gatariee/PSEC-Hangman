class Word:
    ...
    def show(self):
        print(
            f"The current word is {self.word} and the difficulty is {self.difficulty}"
        )


wordlist = [{"word": "kangaroo", "difficulty": "hard"}]


def banner(player: str, session: int, incorrect: int, incorrect_list: list, guess_progress: str) -> str:
    print(
...
def game(player: str, session: int) -> int:
    ...
    while "_" in guess_progress and counter < 5:
        ...
        guess_progress, guess_list, check, incorrect_list = guessCheck(
            ...
            previous_result=guess_progress,
            incorrect=incorrect_list,
        )
        if not check:
            counter += 1
        banner(player, session, counter, incorrect_list, guess_progress)
    return len(guess_progress.replace(" ", "").replace("_", ""))


def guessCheck(
    word: str, guess: str, previous_guesses: str, previous_result: str, incorrect: list
) -> str:
    str = ""
    check = False
    for letter in word:
